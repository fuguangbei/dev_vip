...
function commercial_show(commercial_object) {
    ...
    var cached_img = localStorage.getItem(commercial_object.cover)//从本地存储读取数据

    $container.append($elem); //添加请求回的内容到容器

    $elem.find('img').attr('data-src', cover_url); //更换commercial图片地址

    $elem.find('img').on('load', function () {//图片加载完后执行
        var data = img2dataURI(this)//转字符串
        localStorage.setItem(commercial_object.cover, data)//健值对形式写入到local storage
        console.log(commercial_object.cover, data);
    })
...
function ad() {
    ...
    var cached_list = $.parseJSON(localStorage.getItem('cached_commercial_list'))//json转成js对象
    if (cached_list !== null) {
...
function feature_list_show(feature_object) {
    ...
    $.ajax({
        ...
        type: 'get',
        success: function () {
            $elem.find('.feature-img').attr('src', cover_url);
        },
        error: function () {
