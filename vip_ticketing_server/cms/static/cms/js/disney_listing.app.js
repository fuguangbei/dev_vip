...
var reload_disney_data = function (id) {
	$.ajax({
		url: '/cms/disney/{0}/'.format(id),
		type: 'get',
...
var render_item = function (obj) {
	var template = $('#disney-ticket-template').html(),
		container = $('#tickets')
...
var invoke_inventory_update = function (disney_data, date, current_inventory) {
	render_inventory_modal(disney_data.title, [{
		date: date,
		inventory: current_inventory
	}])
	$('#update-inventory').data('tid', disney_data.id)
	$('#update-inventory').modal()
}

var update_inventory = function (disney_id, date_list, number_list) {
	// console.log('updating ticket {0} inventory on date {1}'.format(disney_id, date))
	$.ajax({
		url: '/cms/disney/update/',
		type: 'post',
